...
import (
	...
	"github.com/verrazzano/rancher/pkg/kubectl"
...
func (cd *clusterDeploy) getYAML(cluster *apimgmtv3.Cluster, agentImage, authImage string, features map[string]bool, taints []corev1.Taint) ([]byte, error) {
	...
	if privateRegistrySecretRef != "" || privateRegistryURL != "" || ecrSecretRef != "" {
		...
		spec, err = secretmigrator.AssemblePrivateRegistryCredential(privateRegistrySecretRef, privateRegistryURL, secretmigrator.ClusterType, cluster.Name, spec, cd.secretLister)
		if err != nil {
			return nil, err
		}
